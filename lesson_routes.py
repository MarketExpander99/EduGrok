...
def check_lesson():
    ...
    try:
        ...
        if activity_type == 'trace_complete':
            ...
            expected = 'mat'  # Hardcoded based on template example; adjust if dynamic
        elif activity_type == 'math_fill':
            expected = '6'  # Example for 2+4; make dynamic if needed
        elif activity_type == 'match_game':
            expected = 'demo_match'  # Stub for game
        ...
        logger.info(f"Lesson {lesson_id} activity {activity_type} for user {session['user_id']}: {'Correct' if is_correct else 'Incorrect'} (response: {response}, expected: {expected})")
...
def generate_lesson():
    ...
    trace_word = 'sun' if subject == 'language' else None
    spell_word = 'sun' if subject == 'language' else None
    sound = '/sʌn/' if subject == 'language' else None
    mc_question = 'What is the correct spelling?' if subject == 'language' else None
    mc_options = 'sun,son,sunne' if subject == 'language' else None  # Comma-separated
    mc_answer = 'sun' if subject == 'language' else None
    content = f"Learn the {sound} sound with words like sun and star." if subject == 'language' else "Math lesson content."

    try:
        conn = get_db()
        c = conn.cursor()
        c.execute('''INSERT INTO lessons (user_id, grade, subject, content, trace_word, sound, spell_word, 
                     mc_question, mc_options, mc_answer) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
